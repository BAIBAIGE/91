...
Usage:
  ...
  91 reset-password [--user-id ID] [--config PATH]
...
Actions:
  ...
  reset-password  Generate a random 12-character password for an existing user
...
reset_password() {
  ...
  (
    cd "$install_dir"
    VIDEO_CONFIG="${VIDEO_CONFIG:-$install_dir/config.yaml}" ./server reset-password "$@"
  )
}
