type ScanResultIconProps = {
  size?: number;
  className?: string;
};

// Font Awesome Pro 7.3.1 by @fontawesome - https://fontawesome.com/license
// Commercial License, Copyright 2026 Fonticons, Inc.
export function ScanResultIcon({ size = 20, className }: ScanResultIconProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      width={size}
      height={size}
      viewBox="0 0 640 640"
      aria-hidden="true"
      focusable="false"
    >
      <path
        fill="currentColor"
        d="M512 512L128 512C92.7 512 64 483.3 64 448L64 160C64 124.7 92.7 96 128 96L266.7 96C280.5 96 294 100.5 305.1 108.8L343.5 137.6C349 141.8 355.8 144 362.7 144L512 144C547.3 144 576 172.7 576 208L576 448C576 483.3 547.3 512 512 512zM400 320C400 267 357 224 304 224C251 224 208 267 208 320C208 373 251 416 304 416C321.8 416 338.4 411.2 352.7 402.8L391 441.1C400.4 450.5 415.6 450.5 424.9 441.1C434.2 431.7 434.3 416.5 424.9 407.2L386.6 368.9C395.1 354.6 399.9 337.9 399.9 320zM304 272C330.5 272 352 293.5 352 320C352 346.5 330.5 368 304 368C277.5 368 256 346.5 256 320C256 293.5 277.5 272 304 272z"
      />
    </svg>
  );
}
