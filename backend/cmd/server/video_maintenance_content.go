...
func (a *App) cleanupContentDuplicateVideos(ctx context.Context, localDir string, videos []*catalog.Video, deleted map[string]struct{}) (contentDuplicateMaintenanceStats, error) {
	...
	for i := range candidates {
		...
		for windowStart < i && candidates[i].video.DurationSeconds-candidates[windowStart].video.DurationSeconds > videoMaintenanceDurationToleranceSeconds {
		...
		for j := windowStart; j < i; j++ {
			...
			if cmp.IsContentDuplicate() {
				...
				stats.NearMisses++
				log.Printf("[dedupe-maintenance] content near-miss (manual review) left=%s right=%s median_ssim=%.3f min_ssim=%.3f comparisons=%d title_left=%q title_right=%q",
