...
`)
	...
	want := LiveSettings{NightlyStartTime: "04:25"}
...
func TestManagerYAMLValuesWinOverLegacySQLiteValues(t *testing.T) {
	...
	want := LiveSettings{NightlyStartTime: "02:10"}
	if got := manager.LiveSettings(); got != want {
...
func TestManagerReloadPublishesExternalValidChangeAndKeepsLastGoodOnError(t *testing.T) {
	...
	manager.SetApply(func(settings LiveSettings) { applied = append(applied, settings) })
	...
	want := LiveSettings{NightlyStartTime: "06:30"}
	if got := manager.LiveSettings(); got != want {
